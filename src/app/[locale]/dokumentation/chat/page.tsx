...
function DoksChatDE() {
  return (
    <>
      ...
      <p>Verbinden Sie sich per SSH mit Ihrem Server und führen Sie aus:</p>
      ...
      <h2>4. HTTPS aktivieren</h2>
      <p>HTTPS ist für den Produktivbetrieb erforderlich. Das Setup nutzt kostenlose Let&apos;s Encrypt Zertifikate:</p>
      <pre><code>sudo bash /opt/serahrchat/repo/scripts/setup-tls.sh chat.meinefirma.de admin@meinefirma.de</code></pre>
      ...
      <h2>6. Widget einbetten</h2>
      <p>Fügen Sie folgenden Code in Ihre Website ein (vor <code>&lt;/body&gt;</code>):</p>
      <pre><code>{`<script
  ...
  data-color="#2563eb"
      ...
      <p><strong>CORS:</strong> Falls das Widget auf einer anderen Domain eingebettet wird, tragen Sie die Widget-Domain im Admin-Bereich unter Einstellungen als erlaubte Herkunft ein.</p>

      <h2>7. Lokales KI-Modell mit Ollama oder LM Studio (optional)</h2>
...
ollama pull nomic-embed-text`}</code></pre>
      ...
      <h2>8. Backup und Wiederherstellung</h2>
...
bash /opt/serahrchat/repo/scripts/restore.sh`}</code></pre>
...
function DoksChatEN() {
  return (
    <>
      ...
      <h2>3. Installation</h2>
      <p>Connect to your server via SSH and run:</p>
      <pre><code>curl -fsSL https://update.serahr.de/serahrchat/preflight.sh | sudo bash -s -- --install</code></pre>
      <ol>
      ...
      <h2>4. Enable HTTPS</h2>
      <pre><code>sudo bash /opt/serahrchat/repo/scripts/setup-tls.sh chat.mycompany.com admin@mycompany.com</code></pre>
      <p>The script requests a TLS certificate, configures nginx for HTTPS, and sets up automatic certificate renewal.</p>
  ...
  data-color="#2563eb"
      ...
      <p><strong>CORS:</strong> If the widget is embedded on a different domain, add that domain as an allowed origin in the admin panel under Settings.</p>

      <h2>7. Local AI Model with Ollama or LM Studio (optional)</h2>
...
ollama pull nomic-embed-text`}</code></pre>
      ...
      <pre><code>bash /opt/serahrchat/repo/scripts/restore.sh</code></pre>
